...
// Request 自定义请求类型
func DoRequest(verb string, url string, headers map[string]string, body []byte, options *ReqOptions) (*Response, error) {
	sess := getSessionByOptions(options)
...
func (r *ReqInfo) Request() (*Response, error) {
	return DoRequest(r.Verb, r.Url, r.Headers, r.Body, nil)
}

func (r *ReqInfo) RequestWithOptions(options *ReqOptions) (*Response, error) {
	return DoRequest(r.Verb, r.Url, r.Headers, r.Body, options)
}
