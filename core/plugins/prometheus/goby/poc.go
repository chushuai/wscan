...
type PocCollection struct {
	Normal []Poc `json:"normal"`
}
...
//			continue
//		}
//		fmt.Println(string(bytes[0:500]))
//		err = json.Unmarshal(bytes, &poc)
//		if err != nil {
//			logger.Error(err)
//			continue
//		}
//		if poc.ScanSteps == nil && poc.ExploitSteps == nil {
//			//
//			pocCollection := PocCollection{}
//			err = json.Unmarshal(bytes, &pocCollection)
//			if err != nil {
//				logger.Error(err)
//				continue
//			}
//			pocList := pocCollection.Normal
//			for i := range pocList {
//				if pocList[i].ScanSteps == nil && pocList[i].ExploitSteps == nil {
//					continue
//				}
//				pocs = append(pocs, &pocList[i])
//			}
//			continue
//		}
...
// LoadAllPoc 加载全部poc
func LoadOnePoc(data string) (*Poc, error) {
	var poc Poc
	plugCode, _ := base64.StdEncoding.DecodeString(data)
	err := json.Unmarshal(plugCode, &poc)
	if err != nil {
		logger.Error(err)
		return nil, err
	}
