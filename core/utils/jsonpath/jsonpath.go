...
//
// A jsonpath applies to any JSON decoded data using any when
...
func ToMapInterface(origin any) (map[string]any, any, error) {
	switch origin.(type) {
	case string, []byte, []rune:
		var genericObj any
		err := json.Unmarshal([]byte(fmt.Sprintf("%v", origin)), &genericObj)
		if err != nil {
			return nil, origin, errors.New(fmt.Sprintf("jsonpath unmarshal origin[%v] failed: %s", spew.Sdump(origin), err))
		}
		origin = genericObj
	}
	if origin == nil {
		return make(map[string]any), origin, errors.New(fmt.Sprintf("empty origin"))
	}
	m, err := utils.InterfaceToMapInterfaceE(origin)
	return m, origin, err
}

func deepCopyMapRaw(h map[string]any) (map[string]any, error) {
	var newValues = make(map[string]any)
...
func Replace(origin any, path string, replaceValue any) (map[string]any, error) {
...
func ReplaceEx(origin any, path string, replaceValue any) (any, error) {
	var data any
	...
	if err != nil {
		...
		if err != nil {
			return make(map[string]any), errors.New(fmt.Sprintf("cannot parse[%v] to map[str]any / []any: %v", spew.Sdump(err), err))
		}
		data = sliceMaybe
	} else {
		isMap = true
...
type actionFunc func(r, c any, a actions) (any, error)

// a list of action functions to apply one after the other
type actions []actionFunc

// next applies the next action function
func (a actions) next(r, c any) (any, error) {
	return a[0](r, c, a[1:])
}

// call applies the next action function without taking it out
func (a actions) call(r, c any) (any, error) {
	return a[0](r, c, a)
}

type exprFunc func(r, c any) (any, error)

type searchResults []any

func (sr searchResults) append(v any) searchResults {
...
func newScannerWithReplaceValue(path string, replaceValue any) *parser {
...
func (p *parser) parsePath() (err error) {
	for err == nil {
		...
		case scanner.EOF:
			...
			p.add(func(r, c any, a actions) (any, error) { return c, nil })
			return nil
		default:
...
func (p *parser) parseObjAccess() error {
	...
	p.add(func(r, c any, a actions) (any, error) {
...
func (p *parser) parseArray() error {
	var indexes []any // string, int or exprFunc
...
func (p *parser) recSearchParent(r, c any, a actions, acc searchResults) searchResults {
...
func (p *parser) recSearchChildren(r, c any, a actions, acc searchResults, allowReplace ...bool) searchResults {
	if obj, ok := c.(map[string]any); ok {
...
func (p *parser) prepareIndex(index any, column int) actionFunc {
	return func(r, c any, a actions) (any, error) {
		if obj, ok := c.(map[string]any); ok {
...
func (p *parser) prepareSlice(indexes []any, column int) actionFunc {
	return func(r, c any, a actions) (any, error) {
		array, ok := c.([]any)
		if !ok {
...
func (p *parser) prepareUnion(indexes []any, column int) actionFunc {
	return func(r, c any, a actions) (any, error) {
		if obj, ok := c.(map[string]any); ok {
			var values searchResults
...
func indexAsInt(index, r, c any) (int, error) {
	switch i := index.(type) {
	case int:
		return i, nil
	case exprFunc:
...
func indexAsString(key, r, c any) (string, error) {
	switch s := key.(type) {
	case string:
		return s, nil
	case exprFunc:
...
func valuesSortedByKey(m map[string]any) []any {
	if len(m) == 0 {
		return nil
	}
