...
```bash
# 只显示 high 及以上等级
./wscan ws --min-severity high --basic-crawler  --url http://target --html-output report.html

# 排除 baseline 和 fingerprint
...
```yaml
vuln-filter:
    min_severity: "info"        # 最低显示等级: info/low/medium/high/critical
    exclude_vulns:              # 排除的漏洞ID，支持glob
        - "baseline/*"
    include_vulns:              # 只扫描的漏洞ID，支持glob
        - "xss/*"
```
...
| low | dirscan, pathbrute, swagger, baseline(Cookie/CSP等) |
