...
// modified from https://github.com/lucasjones/reggen/blob/master/reggen.go
// changes:
// 1. make printableChars only contains digit and alpha
// 2. generate more than once when op is * or +
...
func (g *Generator) generate(s *state, re *syntax.Regexp) string {
	...
	case syntax.OpRepeat:
		...
		if re.Max > re.Min {
			count = g.rand.Intn(re.Max-re.Min+1) + 2
		}
		if g.debug {
